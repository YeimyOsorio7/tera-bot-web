...
const Recursos = () => {
  ...
  const eliminarRecurso = (id) => {
    ...
    toast.className = 'fixed bottom-4 left-1/2 transform -translate-x-1/2 bg-red-600 text-white px-4 py-2 rounded shadow z-50 animate-fade-in';
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-sky-700 dark:text-sky-300 mb-6">📁 Recursos</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {recursos.map((recurso) => (
          <div key={recurso.id} className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg p-4 shadow">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">{recurso.nombre}</h2>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">Tipo: {recurso.tipo}</p>
            <a
              href={recurso.url}
              target="_blank"
              rel="noopener noreferrer"
              className="w-full flex items-center justify-center gap-2 bg-sky-600 hover:bg-sky-700 text-white font-medium text-sm px-4 py-2 rounded shadow transition"
            >
              Ver recurso
            </a>
            <button
              onClick={() => setModal({ mostrar: true, id: recurso.id })}
              className="mt-2 w-full flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 text-white font-medium text-sm px-4 py-2 rounded shadow transition"
            >
              Eliminar
            </button>
          </div>
      ...
      {modal.mostrar && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-900 rounded-lg p-6 shadow-xl max-w-sm w-full text-center">
            <h2 className="text-lg font-bold text-gray-800 dark:text-white mb-4">¿Está seguro?</h2>
